...
/// it, use `Individual::get_data(&FastNonDominatedSort::rank_key()).unwrap()`.
///
    ...
    pub fn sort(
        ...
        for pi in 0..individuals.len() {
            ...
            if domination_counter[pi] == 0 {
                ...
                individuals[pi].set_data(&FastNonDominatedSort::rank_key(), DataValue::Integer(1));
            }
        }
            ...
            for pi in current_front.iter() {
                ...
                for qi in dominated_solutions[*pi].iter() {
                    ...
                    if domination_counter[*qi] == 0 {
                        next_front.push(*qi);
                        individuals[*qi]
                            .set_data(&FastNonDominatedSort::rank_key(), DataValue::Integer(i + 1));
                    }
                }
            }
            i += 1;
...
mod test {
    ...
    fn test_sorting_2obj() {
        ...
        let result = FastNonDominatedSort::sort(&mut individuals, false).unwrap();
        ...
        for idx in &expected_first {
            assert_eq!(
                individuals[*idx]
                    .get_data(&FastNonDominatedSort::rank_key())
                    .unwrap(),
                DataValue::Integer(1)
            );
        }
