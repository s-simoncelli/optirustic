...
use crate::utils::{
    argmin, DasDarren1998, fast_non_dominated_sort, perpendicular_distance, solve_linear_system,
...
/// The data key where the minimum distance is stored for each [`Individual`].
const MIN_DISTANCE: &str = "minimum_distance";
...
impl NSGA3 {
    ...
    pub fn new(problem: Problem, options: NSGA3Arg) -> Result<Self, OError> {
        ...
        // TODO Number of individuals is set equal to number of ref points for now
        let number_of_individuals = das_darren.number_of_points() as usize;
    ...
    fn evolve(&mut self) -> Result<(), OError> {
        ...
        for (fi, front) in sorting_results.fronts.into_iter().enumerate() {
            if new_population.size() + front.len() <= self.number_of_individuals {
                ...
                // Algorithm 1, step 12. Population is filled up to front l-1
                debug!(
        ...
        if let Some(mut last_front) = last_front {
            // calculate St (new_population and last_front)
            new_population.0.append(&mut last_front);
