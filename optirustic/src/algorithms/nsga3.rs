...
pub struct NSGA3Arg {
    ...
    /// The number of individuals in the population. This defaults to the number of partitions. If
    /// desired, this cna be overwritten, but the size must be (1) larger than the number of
    /// reference points generated by setting the [`NSGA3Arg::number_of_partitions`] and (2) must be
    /// a multiple of `2`.
...
impl NSGA3 {
    ...
    pub fn new(problem: Problem, options: NSGA3Arg) -> Result<Self, OError> {
        ...
        // create population and check its size
        let number_of_individuals = options
            .number_of_individuals
            .unwrap_or(das_darren.number_of_points() as usize);
        let population = Population::init(problem.clone(), number_of_individuals);
        let nsga3_args = options.clone();
        if number_of_individuals < 3 {
            return Err(OError::AlgorithmInit(
                name,
        ...
        if number_of_individuals > das_darren.number_of_points() as usize {
            return Err(OError::AlgorithmInit(
                name,
                format!(
                    concat!(
